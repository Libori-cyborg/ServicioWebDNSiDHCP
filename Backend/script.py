...
def require_key(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if key != API_KEY:
            return jsonify({
                "success": False,
                "output": "❌ No autoritzat: clau incorrecta o absent"
            }), 401
...
def get_config():
    ...
    if key not in CONFIG_FILES:
        return jsonify({
            "success": False,
            "content": "",
            "output": f"Clau de configuració no vàlida: {key}"
        })
...
def get_config():
    try:
        key = request.args.get("key")
        print("🔑 KEY rebuda:", key)

        if not key:
            return jsonify({
                "success": False,
                "content": "",
                "output": "No s'ha rebut cap clau (key)."
            }), 400
...
def save_config():
    try:
        ...
        if not key or not content:
            return jsonify({
                "success": False,
                "output": "Falten camps: path o content"
            }), 400
        ...
        return jsonify({
            "success": True,
            "output": f"✅ Fitxer desat correctament: {path}"
        })

    except Exception as e:
        return jsonify({
            "success": False,
            "output": f"Error desant fitxer: {str(e)}"
        }), 500
